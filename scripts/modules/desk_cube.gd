...
const STEM_TEX := preload("res://assets/modules/cube_stem.png")
...
func _ready() -> void:
	...
	add_child(_nub)
	_stem = Sprite2D.new()
	_stem.texture = STEM_TEX
	_stem.centered = true
	_stem.z_index = 5
	_stem.visible = false
	_stem.texture_filter = CanvasItem.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS
	add_child(_stem)
...
func _process(delta: float) -> void:
	...
	if _drag == "worry" and Time.get_ticks_msec() - _last_worry_ms > 70:
		AudioFeel.stop_held("rub")
	...
	if _face == Face.JOYSTICK:
		...
		if mag > 0.05 and _stem and STEM_TEX:
			var dir: Vector2 = _joy.normalized()
			var cap_at: Vector2 = _cube.position + throw_v
			var stem_a: Vector2 = _cube.position + dir * (_well_r() * 0.04)
			var stem_b: Vector2 = cap_at - dir * (_nub_r() * 0.42)
			var stem_len: float = stem_a.distance_to(stem_b)
			if stem_len > 6.0:
				_stem.visible = true
				_stem.position = stem_a.lerp(stem_b, 0.5)
				_stem.rotation = dir.angle() + PI * 0.5
				var want_w: float = _nub_r() * 1.22
				_stem.scale = Vector2(
					want_w / float(STEM_TEX.get_width()),
					stem_len / float(STEM_TEX.get_height())
				)
		var at_rim := mag > 0.92
		if at_rim and not _joy_rim and _drag == "joy":
			Haptics.selection()
			AudioFeel.play_click(1.05)
		_joy_rim = at_rim
	else:
		_nub.visible = false
...
func _apply_face() -> void:
	...
	_nub.visible = _face == Face.JOYSTICK
	if _stem:
		_stem.visible = _face == Face.JOYSTICK and _stem.visible
	if _dial:
		_dial.visible = _face == Face.GEAR
	queue_redraw()
		...
		if button.pressed:
			...
			if _cube_rect().grow(-10.0).has_point(button.position):
				match _face:
					Face.JOYSTICK:
						_drag = "joy"
						_aim_joy(button.position, true)
					Face.GEAR:
						_drag = "gear"
					Face.WORRY:
						_drag = "worry"
						_last = button.position
						_last_worry_ms = Time.get_ticks_msec()
						Haptics.soft()
						AudioFeel.play_rub(0.42)
					Face.DOTS:
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and not event.pressed:
		_release_at(_to_local(event))
	elif event is InputEventMouseMotion:
		var pos := _to_local(event)
		match _drag:
			"joy":
				_aim_joy(pos, false)
			"gear":
				var a0: float = (_last - _cube.position).angle()
				var a1: float = (pos - _cube.position).angle()
				...
				if d != _gear_detent:
					_gear_detent = d
					Haptics.selection()
					AudioFeel.play_click(randf_range(0.9, 1.1))
			"worry":
				_rub(pos)
			"flip":
				pass
...
func _to_local(event: InputEvent) -> Vector2:
	var local_ev := make_input_local(event)
	if local_ev is InputEventMouse:
		return (local_ev as InputEventMouse).position
	if local_ev is InputEventScreenTouch:
		return (local_ev as InputEventScreenTouch).position
	if local_ev is InputEventScreenDrag:
		return (local_ev as InputEventScreenDrag).position
	return Vector2.ZERO
...
func _aim_joy(pos: Vector2, grabbing: bool = false) -> void:
	var maxr := _max_nub()
	if maxr < 1.0:
		_joy_target = Vector2.ZERO
		return
	if grabbing:
		_joy_grab = pos - (_cube.position + _joy * maxr)
	_joy_target = ((pos - _joy_grab) - _cube.position).limit_length(maxr) / maxr
...
func _rub(pos: Vector2) -> void:
	...
	_last_worry_ms = Time.get_ticks_msec()
	Haptics.soft()
	AudioFeel.play_rub(clampf(0.36 + dist / 80.0, 0.36, 0.50))
...
func _press_dot(pos: Vector2) -> void:
	...
	var best_d := INF
	for i in spots.size():
		var d := pos.distance_to(spots[i])
		if d < best_d:
			best_d = d
			best = i
	if best_d > _cube_size() * 0.26:
		Haptics.soft()
		AudioFeel.play_clack(0.38)
		return
	_dots[best] = not _dots[best]
	if _dots[best]:
		Haptics.rigid()
		AudioFeel.play_clack(0.78)
	else:
		Haptics.medium()
		AudioFeel.play_clack(0.52)
